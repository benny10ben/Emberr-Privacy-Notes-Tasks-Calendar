...
class SyncClient(
    ...
    suspend fun listRemoteMedia(): List<com.ben.emberr.domain.sync.RemoteMediaEntry> {
        return try {
            client.get("$serverUrl/sync/media/list").body<com.ben.emberr.domain.sync.RemoteMediaList>().entries
        } catch (e: Exception) {
